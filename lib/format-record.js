...
var OM_SHORT = 5;
var OM_BUNYAN = 6;
var OM_SHORT_WITH_JSON = 7;
var OM_LONG_WITH_JSON = 8;
var OM_FROM_NAME = {
  'long': OM_LONG,
  'json': OM_JSON,
  'inspect': OM_INSPECT,
  ...
  'short': OM_SHORT,
  'bunyan': OM_BUNYAN,
  'short-with-json': OM_SHORT_WITH_JSON,
  'long-with-json': OM_LONG_WITH_JSON
};
...
/**
 * @param {int|string} level is the level of the record.
...
module.exports = function formatRecord(rec, opts) {
  ...
  case OM_LONG:
    ...
    delete rec.req_id;
    oneLineMsg = getOneLineMessage(details);
  ...
  case OM_SIMPLE:
    ...
    return format('%s - %s\n',
      ...
      rec.msg);
  case OM_SHORT_WITH_JSON:
    delete rec.v;
    delete rec.hostname;
    short = true;
    oneLineMsg = getOneLineMessage(details)
    detailString = stylize((details.length ? details.join('\n  --\n') + '\n' : ''), 'brightBlack');
  ...
  function getOneLineMessage(details) {
    var onelineMsg;
    if (rec.msg.indexOf('\n') !== -1) {
      onelineMsg = '';
  ...
  function getJson() {
    ...
    var result = JSON.stringify(rec, null, opts.jsonIndent);
    if (result === '{}') {
      result = '';
    }
    return result + '\n';
